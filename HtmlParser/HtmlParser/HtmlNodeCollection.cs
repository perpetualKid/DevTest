...
namespace HtmlParser
{
    ...
    /// </summary>
    public class HtmlNodeCollection : Collection<HtmlNode>
    {
        ...
        public HtmlNodeCollection GetByName(string name, bool recursive)
        {
            ...
            foreach (HtmlNode node in this)
            {
                ...
                if (null != element)
                {
                    if (string.Compare(element.Name, name, StringComparison.OrdinalIgnoreCase) == 0)
                    {
                        results.Add(node);
                    }
                    if (recursive)
        ...
        public HtmlNodeCollection FindByAttributeNameValue(string attributeName, string attributeValue, bool recursive)
        {
            ...
            foreach (HtmlNode node in this)
            {
                ...
                if (null != element)
                {
                    ...
                    {
                        if (attribute != null && string.Compare(attribute.Value, attributeValue, StringComparison.OrdinalIgnoreCase) == 0)
                        {
                            results.Add(node);
                        }
                    }
